...
#define UCK1                    (3)       //PC0 Channel 10
  ...
  #define RE_RELASE_VOLTAGE_MIN   (13.0)
...
void Cap_Init(void) {
  ...
  double FUNC_NEW_Get_Voltage(void)
  {
	...
	if (ADC_val[0]  != 0){
	new_cap_voltage = 1.2*0.002 * ADC_val[3] / ADC_val[0] + last_cap_voltage*0.998;
	}
...
void Cap_Run(void) {
	...
	Cap_Ctr();
	Cap_State();
	//custom_data.masks = 0xC0 | ((1 << ((int)(((VAL__CAP_VOLTAGE*VAL__CAP_VOLTAGE - 121) / (22*22 - 121)) * 6 + 1))) - 1);
}
...
void LED_Show_SuperCap_Voltage(uint8_t flag)
{
	...
	if (VAL__CAP_VOLTAGE < 11)
		...
		HAL_GPIO_WritePin(GPIOG, 0x1fe, GPIO_PIN_SET);
		int unlight = 7 - (int) ( ((VAL__CAP_VOLTAGE * VAL__CAP_VOLTAGE - 121) / (22*22 - 121)) * 6);
